    ...
    <div className="min-h-screen bg-[#FDFCF8] p-4 md:p-12 font-serif text-stone-800">
      <div className="max-w-6xl mx-auto bg-white rounded-[3rem] p-8 md:p-12 shadow-sm border border-stone-100">
        ...
        <header className="text-center mb-10">
          ...
          <p className="text-stone-400 italic mt-2 text-sm">Organizando quem serve e quem pratica</p>
          
          <div className="mt-8 inline-block bg-stone-50 p-2 rounded-2xl border border-stone-100">
            <select 
              value={idRetiro} 
              onChange={(e) => setIdRetiro(e.target.value)}
              className="bg-transparent outline-none text-sm font-medium text-stone-600 px-4 cursor-pointer"
            >
        ...
        <div className="space-y-4">
          <div className="hidden md:grid grid-cols-12 px-6 py-2 text-[10px] uppercase tracking-widest text-stone-400 font-bold">
            <div className="col-span-4">Nome / E-mail</div>
            <div className="col-span-3 text-center">Ocupação Física</div>
            <div className="col-span-5 text-right">Nível de Acesso no Sistema</div>
          </div>
            ...
            equipe.map(u => (
              <div key={u.id} className="grid grid-cols-1 md:grid-cols-12 items-center p-6 bg-stone-50/30 border border-stone-50 rounded-[2rem] hover:border-amber-100 transition-all gap-4">
                ...
                <div className="col-span-1 md:col-span-4">
                  <p className="font-medium text-stone-800">{u.nome}</p>
                ...
                <div className="col-span-1 md:col-span-3 flex justify-center">
                  ...
                  >
                    {u.ocupa_vaga ? '🏠 OCUPA VAGA' : '☁️ STAFF / ISENTO'}
                  </button>
                </div>

                {/* Seleção de Nível de Acesso - INCLUÍDO COZINHA */}
                <div className="col-span-1 md:col-span-5 flex flex-wrap justify-end gap-1">
                  {['PARTICIPANTE', 'COZINHA', 'LOGISTICA', 'FINANCEIRO', 'ADMIN'].map(papel => (
                    <button
                      key={papel}
                      onClick={() => mudarPapel(u.id, papel)}
                      className={`px-3 py-1.5 text-[9px] font-bold rounded-full border transition-all ${
                        u.nivel_acesso === papel 
        ...
        <footer className="mt-12 pt-8 border-t border-stone-50 text-center">
          <p className="text-[10px] text-stone-300 italic">
            Participantes marcados como "Staff / Isento" não são contabilizados nas vagas do Dashboard nem na projeção financeira.
          </p>
        </footer>
      </div>
    </div>
